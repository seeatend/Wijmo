...
import DeleteColumnDropdown from './../common/DeleteColumnDropdown';
...
export default class Panel extends React.Component {
  constructor(props) {
    ...
    this.getDeleteColumnDropdown = this.getDeleteColumnDropdown.bind(this)
  ...
  retrieveState() {
    ...
    if (linkState) {
      ...
      link_state.forEach(link => {
        this.setupDropdowns(link.binding, link.table, link.column)
      })
    }
  }
  ...
  setupDropdowns(binding_column, link_table_name, link_table_column) {
    let link_table_dropdowns = []
    const views_ref = firebase.ref().child(link_table_name)
    views_ref.once('value', (snapshot) => {
      let index = 0
      snapshot.forEach(childSnapshot => {
        let childData = childSnapshot.val();
        link_table_dropdowns.push({
          id: link_table_column=='id'?childSnapshot.key:childData[link_table_column]
        });
      });
    })

    //Remove duplicate element
    // console.log(link_table_dropdowns)
    ...
    if (flex) {
      ...
      columns.forEach((column) => {
        ...
        if (binding == binding_column) {
          column.dataMap = new DataMap(link_table_dropdowns, 'id', 'id')
        }
      })
    }
    if (view) {
  ...
  getAddColumnDropdown() {
    const { currentView } = this.state
    return (
      <AddColumnDropdown table={TABLE_KEY} currentView={currentView}/>
    )
  }

  getDeleteColumnDropdown() {
    const { currentView } = this.state
    return (
      <DeleteColumnDropdown table={TABLE_KEY} currentView={currentView}/>
    )
  }
